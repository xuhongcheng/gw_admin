...
<!--            </el-form-item>-->
            <el-form-item label="">
              <el-input size="small" v-model="searchData.name" placeholder="请输入课程名称"></el-input>
            </el-form-item>
            <el-form-item label="">
...
<!--            </el-table-column>-->
            ...
            <el-table-column label="状态" width="100" align="center">
              <template slot-scope="scope">
                <div v-if="!Boolean(scope.row.record)">
                  <span class="warning-state">待审核</span>
                </div>
                <div v-else>
                  <span v-if="Boolean(scope.row.state)" class="success-state">已通过</span>
                  <span v-else class="error-state">已拒绝</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right" width="160px" align="center">
              <template slot-scope="scope">
                ...
                <div v-else>
                  <el-button size="mini" type="primary" @click="handleEdit(scope.$index, scope.row.id)">通过</el-button>
...
<script>
  export default {
    ...
    methods: {
      ...
      hanldeAdd() {  // 添加
        this.$router.push('/basic/college/add')
      },
      showModal() {
        this.visible = true
      },
      ...
      handleEdit(index, id) {
        this.tableData[index].record = 1
        this.tableData[index].state = 1
        this.$message.success('通过成功！')
        // this.tableData[index].state = 1
        // this.$router.push('/basic/college/' + id)
      },
      handleSuccess(data) {
        console.log(data)
        this.visible = false
